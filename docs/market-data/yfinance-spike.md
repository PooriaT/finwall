...
| Quote support | Uses Yahoo quote endpoint and maps `regularMarketPrice` plus currency into `MarketPrice`. | Uses a timeout-aware `Ticker(...).history(period="5d", interval="1d", timeout=...)` call and maps the latest close into `MarketPrice`. | Sufficient for a small experimental adapter, but less quote-specific than the current Yahoo provider. |
